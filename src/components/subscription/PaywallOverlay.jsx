...
// Third-party (read-only/childcare) members never own the family's subscription —
// showing them purchase options is both confusing and pointless, since only a
// parent's App Store/Google Play account can actually buy it. They get a plain
// status message instead, with a way out since there's nothing else to do here.
function NonParentBlock() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col items-center justify-center px-6 text-center">
      ...
      <h1 className="text-xl font-bold text-gray-900 mb-2">This family doesn't have an active subscription</h1>
      <p className="text-gray-500 max-w-xs leading-relaxed mb-6">
        Ask a parent in the family to subscribe in the Canopy app — you'll get access as soon as they do.
      </p>
      <button
        type="button"
        onClick={() => signOut()}
...
export default function PaywallOverlay() {
  ...
  const { reload, isParent } = useFamily()
  ...
  useEffect(() => {
    if (!needsPaywall || !native || !isParent) return
    getCurrentOffering().then(setOffering)
  }, [needsPaywall, native, isParent])

  if (!needsPaywall || unlockedLocally) return null

  if (!isParent) return <NonParentBlock />
