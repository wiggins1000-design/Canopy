// Vendored from @capacitor-community/speech-recognition@7.0.1 (ios/Plugin/Plugin.swift).
//
// The npm package only ships CocoaPods packaging (no Package.swift), so Capacitor's SPM
// auto-wrapper silently drops it from ios/App/CapApp-SPM/Package.swift instead of compiling
// it in — every call from JS failed at the bridge with "plugin not implemented on iOS" even
// though `cap sync` reported the plugin as found. Compiling the source directly into the App
// target sidesteps the SPM/CocoaPods packaging problem entirely. Keep this in sync with the
// npm package if it's ever upgraded — the JS side (registerPlugin('SpeechRecognition', ...))
// is unchanged and expects this exact class/method surface.
import Foundation
import Capacitor
import Speech
